...
class AzureDatalakeExt {
    ...
    cache(props, parserOptions = {}) {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            try {
                const { url, table, partitionKey, rowKey, types, replaceIfExists } = props;
                let { overwrite, append } = props;
                overwrite = overwrite === undefined ? false : overwrite;
                append = append === undefined ? false : append;
                if (replaceIfExists) {
                ...
                try {
                    stream_1.pipeline(yield loaders_1.fromAzureDatalake({ url }), transforms_1.CSVStreamToKeywordObjects(), transforms_1.transformKeywordObjects({
                        transformer: (acc, row) => {
                        ...
                        initial: []
                    }), terminators_1.toAzureDataTables({ table, overwrite, append }), err => {
                        if (err)
                            return reject(err);
            ...
            try {
                ...
                yield new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
                    stream_1.pipeline(yield loaders_1.fromAzureDatalake({ url }), transforms_1.CSVStreamToKeywordObjects(), transforms_1.applyMutations({
                        ...
                        modifications,
                        report
                    }), transforms_1.keywordArrayToCSV({ parserOptions: { delimiter } }), yield loaders_1.toAzureDatalake({ url, replace: true }), err => {
                        if (err)
                            return reject(err);
                        resolve(true);
                    });
                ...
                try {
                    ...
                    try {
                        stream_1.pipeline(yield loaders_1.fromAzureDatalake({ url }), transforms_1.CSVStreamToKeywordObjects({ onFirstChunk }), transforms_1.addColumns({
                            columns
                        }), transforms_1.keywordArrayToCSV({ parserOptions: detectedParserOptions }), yield loaders_1.toAzureDatalake({ url, replace: true, parserOptions: detectedParserOptions }), err => {
                            if (err)
                                return reject(err);
                            resolve(true);
                        });
